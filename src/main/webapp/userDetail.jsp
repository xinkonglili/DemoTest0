...
<form class="layui-form layui-form-pane" id="formdata" method="post" enctype="multipart/form-data">
    ...
    <div class="layui-form-item" style="margin-top: 15px">
        <div class="layui-input-block">
            <button class="layui-btn" lay-submit="" lay-filter="uploadpic" id="uploadpic">提交修改</button>
        </div>
    </div>
    <input type="hidden" id="images" disabled>
    <input type="hidden" id="beforeCompanyId" disabled>
    <input type="hidden" id="beforeDepartmentId" disabled>
    <input type="hidden" id="departmentName" disabled>
        ...
        layui.use('form', function () {
            ...
            form.on('submit(uploadpic)', function (data) {
                layui.use('jquery', function () {
                    ...
                    $.ajax({
                        type: 'post',
                        url: 'userServlet?action=modifyUser', // ajax请求路径
                        data: uploaddata,
                        async: false,
                        dataType:"json",
                        contentType: false,
            ...
            setTimeout(function () {
                var companyId=$('#beforeCompanyId').val();
                var departmentId=$('#beforeDepartmentId').val();
                var departmentName=$('#departmentName').val();
                $('#company').val(companyId);
                $('#department').append(new Option(departmentName, departmentId))
                $('#department option:first').prop("selected",'selected')
                var images=$('#images').val();
                if(images.length>0){
                    let arr = images.split(',');
                    $.each(arr,function (index,obj) {
                        var $div=$("<div class=\"img-item\" style=\"float: left;\">\n" +
                            "            <span class=\"delete\" onclick='remove(this)'>x</span>\n" +
                            "            <img src=\""+obj+"\" class=\"img\">\n"+
